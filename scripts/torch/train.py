...
# python train.py --img-list /home/aranem_locale/Desktop/MICCAI_2023/experiments/VoxelMorph_rigid/train_list.txt --seg-list /home/aranem_locale/Desktop/MICCAI_2023/experiments/VoxelMorph_rigid/train_seg.txt
...
def train():
    ...
    if args.atlas:
        ...
        generator = vxm.generators.scan_to_scan(
            train_files, seg_files, batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)
    ...
    for epoch in range(args.initial_epoch, args.epochs):
        ...
        # for step in tqdm(range(args.steps_per_epoch)):
            ...
            if args.seg:
                ...
                y_true.append(segs[1])

                # For CEL instead of VxM Dice --> Switch again so CEl gets arguments correctly
                y_pred_ = y_pred[-1].clone()
                y_pred = [*y_pred[:-1], segs[1].squeeze(1).long(), torch.nn.functional.softmax(y_pred_, dim=1).float()]
                y_true[-1] = y_pred_
            ...
            if args.seg:
                ...
                # _, channel_dices_per_batch = mean_dice_coef(segs[1], y_pred[-1], 1, True) # If dice loss is used
                _, channel_dices_per_batch = mean_dice_coef(segs[1], y_pred[-1].argmax(1), 1, True) # If CEL is used instead of Dice loss
            ...
            if not args.seg:
                _, channel_dices_per_batch = mean_dice_coef(segs[1], segs[0], 1, True)
                mean_dice_nm = [np.mean(v) for _, v in channel_dices_per_batch.items()] # Dice between not moved and fixed segmentation
                dice_nm.append(mean_dice_nm)
        ...
        if not args.seg:
            ...
            print(' - '.join((epoch_info, time_info, loss_info, dice_info, dice_info_nm)), flush=True)
        else:

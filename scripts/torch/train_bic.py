...
"""
Example script to train a VoxelMorph model using BiC method.
...
def train_bic():
    ...
    train_files_train, train_files_val = [item for sublist in train_files_train for item in sublist], [item for sublist in train_files_val for item in sublist]
    seg_files_train, seg_files_val = [x.replace('imagesTr', 'labelsTr') for x in train_files_train], [x.replace('imagesTr', 'labelsTr') for x in train_files_val]
    ...
    if args.use_bias:
        generator_train = vxm.generators.scan_to_scan(
            train_files_train, seg_files_train, batch_size=args.batch_size, bidir=False, add_feat_axis=add_feat_axis)
        generator_val = vxm.generators.scan_to_scan(
            train_files_val, seg_files_val, batch_size=args.batch_size, bidir=False, add_feat_axis=add_feat_axis)
    else:   # --> Use all images not only 80% for the first task, as we on't train the bias layer
        generator_train = vxm.generators.scan_to_scan(
            train_files, seg_files, batch_size=args.batch_size, bidir=False, add_feat_axis=add_feat_axis)

    # extract shape from sampled input
    inshape = next(generator_train)[0][0].shape[1:-1]
    ...
    if args.load_model:
        ...
        model = vxm.BiC.BiC.load(args.load_model, device)
    else:
        model = vxm.BiC.BiC(
            inshape=inshape,
    ...
    # training loops
    for ii in range(2):
        # -- ii == 0 --> train U-Net; ii == 1 --> train BiasField -- #
        if ii==0:
            # -- Freeze the Bias Layer -- #
            for p in model.unet_model.parameters():
                p.requires_grad = True
            for p in model.bias_layer.parameters():
                p.requires_grad = False
        else:
            # -- If trained on first task, there is no bias layer necessary to train -- #
            if not args.use_bias:
                model.save(os.path.join(model_dir, '%04d_%02d.pt' % (args.epochs, ii))) # <-- Otherwise restoring fails..
                continue
            # -- Freeze the network -- #
            for p in model.unet_model.parameters():
                p.requires_grad = False
        ...
        for epoch in range(args.initial_epoch, args.epochs):
            ...
            if epoch % 20 == 0:
                model.save(os.path.join(model_dir, '%04d_%02d.pt' % (epoch, ii)))
            ...
            # for step in tqdm(range(args.steps_per_epoch)):
                ...
                inputs, y_true, segs = next(generator_train) if ii==0 else next(generator_val)
            ...
            # print epoch info
            ii_info = 'Training (Bias Field)' if ii==1 else 'Training (U-Net)'
            epoch_info = 'Epoch %d/%d' % (epoch + 1, args.epochs)
            time_info = '%.4f sec/step' % np.mean(epoch_step_time)
            losses_info = ', '.join(['%.4e' % f for f in np.mean(epoch_loss, axis=0)])
            loss_info = 'loss: %.4e  (%s)' % (np.mean(epoch_total_loss), losses_info)
            dice_info = 'dice: %.4f' % (np.mean(dice))
            alpha_info = 'alpha (bias): %.4f ' % (model.bias_layer.alpha.item())
            beta_info = 'beta (bias): %.4f ' % (model.bias_layer.beta.item())
            print(' - '.join((ii_info, epoch_info, time_info, loss_info, dice_info, alpha_info, beta_info)), flush=True)

        # final model save
        model.save(os.path.join(model_dir, '%04d_%02d.pt' % (args.epochs, ii)))
